...
const definitions: readonly { name: string; description: string; parameters: ParameterSchemaSpec; parse: (raw: unknown) => unknown }[] = [
  ...
  { name: 'context_repo_map', description: scope + 'Discover current snapshot/version and bounded path-sorted receipts, including JSON/README. Optional canonical path is a direct lookup, exclusive with limit/cursor. A confirmed optional-cache write may add blockId when it fits the output budget. ' + coverage,
