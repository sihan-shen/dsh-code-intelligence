...
- 当前执行授权不允许修改宿主工具包。F2 经用户确认按 Native 工具调用验收，通过已有公开 around hook / finalizeContent 桥接；PTC `run_code` 的 message-only 失败不在本阶段支持范围。模型呈现及报告必须说明该边界，不能宣称所有传输均满足结构化错误合同。
