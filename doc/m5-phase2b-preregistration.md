...
>
> **运行后澄清（不改运行前冻结内容）**
> 1. **归因粒度**：F1（中性前言）、F2（转发产品真实 `systemPrompt` 段）与 F4（提问措辞去工具化）在
>    同一轮同时启用，三者都改变模型看到的提示。因此本轮的准确率翻转只能归因于"这三项提示构念
>    修复的合集"，**不能单独归因于措辞**。要拆分贡献必须另立预注册、分别开关 F1/F2/F4 重跑；
>    本轮没有做这个消融。
> 2. **主终点口径**：§5 冻结的主终点是 neutral 子集**按任务多数票**后再比较，不是运行级池化，
>    且 §7 规定两轮复制不得合并统计。按预注册的单轮 N=3 口径分别重算，`agent-comparison-v2.json`
>    与首轮 `agent-comparison-v2-run1.json` 各自三臂 neutral 多数票均为 **19/19 = 100.0%**；
>    round-2 `default` 的池化 `56/57` 仅来自 `decl-01` 在 3 次中失败 1 次（2/3，该任务多数票仍
>    正确）。修复后的 `agent-baseline.mjs` 直接输出 `byVocabulary.neutralMajority` 作为主终点；
>    历史报告只含池化字段，本轮数字不重写。
