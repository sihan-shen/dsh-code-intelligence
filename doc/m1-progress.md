...
| F3 有界扫描与读取 | gpt-6-astra | 进行中 | F1 接口已交付；独立 P0 reader/scanner/line-map |
| F4 AST 文件事实 | gpt-5.6-sol | 进行中 | F1 接口已交付；按覆盖表与 fixture 实现 |
| F5 两阶段构建 | gpt-6-astra | fixture 与终结器开发中 | 基于已确定接口并行开发，完成仍依赖真实 F3/F4 集成 |
